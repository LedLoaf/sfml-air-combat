...
void ParticleNode::addParticle(sf::Vector2f position)
{
	...
	particle.color = Table[static_cast<unsigned int>(mType)].color;
	particle.lifetime = Table[static_cast<unsigned int>(mType)].lifetime;
...
void ParticleNode::computeVertices() const
{
	...
	for (const Particle& particle : mParticles)
	{
		...
		float ratio = particle.lifetime.asSeconds() / Table[static_cast<unsigned int>(mType)].lifetime.asSeconds();
